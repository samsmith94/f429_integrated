...
SPIClass SPI_5(PF9, PF8, PF7); // MOSI, MISO. SCLK
...
void MCP48xx<BITS_RES>::updateDAC() {
    ...
    SPI_5.beginTransaction(SPISettings(20000000, MSBFIRST, SPI_MODE0));

    if (isAActive) {
        digitalWrite(cs, LOW); //select device
        SPI_5.transfer16(command[Channel::A]); // sent command for the A channel
        digitalWrite(cs, HIGH); //deselect device
    }
    if (isBActive) {
        digitalWrite(cs, LOW);
        SPI_5.transfer16(command[Channel::B]); // sent command for the B channel
        digitalWrite(cs, HIGH);
    }

    SPI_5.endTransaction();
}
